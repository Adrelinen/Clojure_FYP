(ns scrape1
  ...
  (:use seesaw.core))
    ...
    (def searchterm(input "Please enter a card: "))
    ...
    (defn MintCard [searchterm]  
            ...
            (def ^:dynamic *base-url* listing ) ;;research what the asterisks and carat are for/ what's their significance
       ...
       (defn Madhouse [searchterm] 
            ...
            (defn hn-price []
                ;;CSS selector for matching all links inside of table elements that have the CSS class “price” 
                ;;html/select parses the nested data structure for matching entries
            (remove #{:0} (map html/text (html/select (fetch-url *base-url*) [:span.inc (html/nth-child 1)])))
                )
            ...
            ;;(def search (button :text "Search"))

            ;;(listen search :action (fn [e] ((print e)(print-listings-and-prices))))

            ;;(invoke-later
                ;;(-> (frame :title "Card Comparer",
